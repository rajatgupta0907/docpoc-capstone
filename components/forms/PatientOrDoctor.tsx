...
async function saveSpecialtyType(profileType: string, userId: string,setSuccessMessage: React.Dispatch<React.SetStateAction<boolean>>) {
...
const PatientOrDoctor = ({userId}:Props) =>{
    ...
    return (
        <main className='mx-auto flex max-w-3xl flex-col justify-start px-10 py-20'>
            ...
            {selectedSpecialty && (
                <button onClick={() => saveSpecialtyType(selectedSpecialty, userId,setShowSuccessMessage)}>Save</button>
            )}
            {showSuccessMessage && (
