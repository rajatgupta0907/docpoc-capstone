"use client";
import { rejectcontactUs } from '@/lib/actions/contact.actions';
...
const Page = ({ params }: { params: { id: string } }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (validateForm()) {
      try {
        const sendMessage = await rejectcontactUs({ id:doctorId, subject: formData.subject, message: formData.message });
        if (sendMessage) {
    ...
    <>
      ...
      <section className="bg-white dark:bg-gray-900">
        <div className="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
          <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">Reject Doctor</h2>
          <p className="mb-8 lg:mb-16 font-light text-center text-gray-500 dark:text-gray-400 sm:text-xl">Give Reason Why</p>
        ...
        <Link
                  href={`/our-admin-doctor-view/${params.id}`}
